...
const base = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../trade-os/reference-3030');
...
root.walkAtRules('font-face', rule => {
  rule.walkDecls('src', decl => { decl.value = decl.value.replaceAll('/fonts/','/reference-3030/fonts/'); });
});
await fs.writeFile(path.join(base,'reference.css'), '/* Generated from the localhost:3030 source. Scoped to these five screens. */\n'+root.toString());
