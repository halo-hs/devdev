...
try {
  ...
  await fs.writeFile(harness, `export { toast } from 'sonner'\nexport { showToast, clearToasts } from '@trade-os/operations/components/toast/toastStore'\n`)
  try {
    ...
    for (const [id, kind, tone, message, shape] of [
      ...
      await save(node, { id, group: '토스트', title: (kind === 'main' ? '업무 공통' : '운영 화면') + ' · ' + ({success:'완료',error:'오류',info:'액션 포함'}[tone]), shape, source: 'shipments.html', origin: 'source-component-preview', sourceCode: kind === 'main' ? 'src/main.tsx:21' : 'trade-os/operations/components/toast/ToastHost.tsx' })
    }
