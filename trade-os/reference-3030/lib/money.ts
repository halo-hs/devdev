...
export function formatScaledMoney(
  ...
  let fraction =
    digits > 0
      ? (rounded % displayScale).toString().padStart(digits, "0")
      : "";
...
export function formatScaledDecimal(
  ...
  const sign = negative && rounded !== BigInt(0) ? "-" : "";
  return fraction ? `${sign}${wholeText}${parts.decimal}${fraction}` : `${sign}${wholeText}`;
}
...
export function formatScaledPct(
  ...
  let formatted = formatScaledMoney(value, scale, null, locale, maximumFractionDigits);
  const decimalIndex = formatted.lastIndexOf(parts.decimal);
  if (decimalIndex >= 0) {
    while (formatted.endsWith("0")) formatted = formatted.slice(0, -1);
    if (formatted.endsWith(parts.decimal)) formatted = formatted.slice(0, -parts.decimal.length);
  }
