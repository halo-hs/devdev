...
export function financeDecimalMagnitude(raw: string | null | undefined): bigint {
  const value = String(raw ?? "0");
  if (!FINANCE_DECIMAL_PATTERN.test(value)) return BigInt(0);
  const negative = value.startsWith("-");
  const unsigned = negative ? value.slice(1) : value;
  const [whole, fraction = ""] = unsigned.split(".");
  const scaled = BigInt(whole) * FINANCE_DECIMAL_SCALE
 ...
 */
export function decimalMagnitude(raw: string | null | undefined, scale: bigint): bigint | null {
  ...
  const unsigned = negative ? value.slice(1) : value;
  const [whole, fraction = ""] = unsigned.split(".");
  const magnitude = BigInt(whole) * scale + BigInt(fraction.padEnd(scaleDigits, "0") || "0");
...
export function amountCell(raw: string | number | null | undefined): AmountCell {
  const value = typeof raw === "number" && Number.isFinite(raw) ? String(raw) : String(raw ?? "");
...
export function ratioPermyriad(raw: string | null | undefined): bigint | null {
  ...
  const rounded = (absolute + RATIO_PERMYRIAD_DIVISOR / BigInt(2)) / RATIO_PERMYRIAD_DIVISOR;
